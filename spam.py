...
class Service:
    ...
    def __init__(self, service_name: str, url: str, data: dict, phone: str,
                 timeout: int):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(self.url,
                    ...
                    print(
                        f"{' ' : >40}SERVICE: '{self.service_name}' - STATUS: {result}"
                    )
            except Exception as e:
                print(
                    f"{' ' : >40}SERVICE: '{self.service_name}' - ERROR: {e}")
    ...
    async def update(self):
        if self.time == 0:
            ...
            await self.request()
            print(
                f"{' ' : >40}TIMEOUT: '{self.service_name}' for {self.time}sec."
            )
        else:
            self.time -= 1
...
class Services:
    ...
    def import_dict(self, service_dict, phone):
        for service_name, service_data in service_dict.items():
            formatted_phone = format_phone(phone,
                                           service_data["phone_pattern"])

            for key, value in service_data["data"].items():
                try:
                    if "%PHONE%" in value:
                        service_data["data"][key] = value.replace(
    ...
    async def start(self, timer):
        while timer > 0:
            await asyncio.gather(*(service.update()
